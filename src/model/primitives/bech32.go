...
func convertBech32Bits(data []byte, fromBits, toBits uint8, pad bool) ([]byte, error) {
...
func EncodeFromBase256(hrp string, data []byte) (string, error) {
	converted, err := convertBech32Bits(data, 8, 5, true)
	if err != nil {
		return "", err
	}
...
func DecodeToBase256(bech string) (string, []byte, error) {
	...
	converted, err := convertBech32Bits(data, 5, 8, false)
	if err != nil {
		return "", nil, err
	}
