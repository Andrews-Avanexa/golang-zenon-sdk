...
const (
	...
	hashBits   = len(primitives.Hash{}) * 8
...
func (tab *Table) Bootstrap(nodes []*Node) {
	...
	for _, n := range nodes {
		cpy := *n
		cpy.sha = primitives.BytesToHashPanic(crypto.Keccak256(n.ID[:]))
...
func (tab *Table) Lookup(targetID NodeID) []*Node {
	var (
		target         = primitives.BytesToHashPanic(crypto.Keccak256(targetID[:]))
...
func (tab *Table) closest(target primitives.Hash, nresults int) *nodesByDistance {
